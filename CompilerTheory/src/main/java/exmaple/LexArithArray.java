...
<float> --> { <digit> }+ "." { <digit> }+
<floatE> --> <float> (exmaple.E|e) [+|-] { <digit> }+
...
exmaple.Id        identifiers
exmaple.Int       integers
...
exmaple.E          float parts ending with exmaple.E or e
...
public abstract class LexArithArray extends IO
{
	...
	// is greater than or equal to that of exmaple.Id.

	// The following variables of "exmaple.IO" class are used:
	...
	public static State state; // the current state of the FA FA的当前状态
	...
	//使用FA的驱动程序提取下一个令牌。
	// If an invalid token is found, issue an error message. 如果找到无效的令牌，请发出错误消息。
